...
/// Immutable format identity. Native OTAP is not a byte encoding.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct PdataFormat(Option<ResolvedCodec>);
...
impl BatchPlan {
    ...
    pub fn new(format: PdataFormat, profile: BatchProfile, preserve: bool) -> Result<Self, Error> {
        ...
        if !preserve && let Some(codec) = format.0 {
            for signal in format.signals() {
    ...
    pub fn batch(
        ...
        let result = match self.working.0 {
            None => {
                let records = inputs
                    .into_iter()
                    .map(|input| {
                ...
                BatchingOutput {
                    batches: result
                        ...
                        .map(|(bytes, weight)| {
                            ...
                            if self.profile.sizer == BatchSizer::Items {
                                encoded = encoded.with_item_count(weight);
                            }
                            (encoded.into(), weight)
                        })
                        .collect(),
