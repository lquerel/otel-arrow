...
impl<PData> ReceiverWrapper<PData> {
    ...
    pub async fn start(
        ...
        self.start_with_codec_executors(
            runtime_ctrl_msg_tx,
            pipeline_completion_msg_tx,
            metrics_reporter,
            node_interests,
            CodecExecutors::default(),
        )
        .await
    }

    pub(crate) async fn start_with_codec_executors(
        self,
        runtime_ctrl_msg_tx: RuntimeCtrlMsgSender<PData>,
        pipeline_completion_msg_tx: PipelineCompletionMsgSender<PData>,
        metrics_reporter: MetricsReporter,
        node_interests: Interests,
        codec_executors: CodecExecutors,
    ) -> Result<TerminalState, Error> {
        match (self, metrics_reporter) {
            (
                ...
                effect_handler.set_codec_executor(codec_executors.local.clone());
            ...
            (
                ...
                effect_handler.set_codec_executor(codec_executors.shared.clone());
                effect_handler.set_source_tagging(source_tag);
                effect_handler.set_capture_policy(capture_policy);
                effect_handler
                    .core
