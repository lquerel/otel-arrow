...
impl DurableBuffer {
    ...
    async fn handle_data(
        ...
        // Returns (context, result, item_count) - item count tracks individual
        ...
        let (context, ingest_result, item_count): (_, Result<(), _>, u64) =
                    ...
                    == Some(&otel_arrow_dfe_pdata::codec::PdataEncoding::OTLP)
            {
                ...
                let otlp_bytes = otel_arrow_dfe_pdata::OtlpProtoBytes::new_from_bytes(
                    signal_type,
                    payload.into_encoded_bytes().expect("OTLP storage input"),
                );
                match OtlpBytesAdapter::new(otlp_bytes) {
                    ...
                    Err((e, original_bytes)) => {
                        ...
                        let nack_pdata = OtapPdata::new(context, OtapPayload::from(original_bytes));
                        effect_handler
                ...
                let result = engine
                    .ingest(&adapter)
                    .await
                    .map_err(|error| (error, OtapPayload::from(adapter.into_inner())));
