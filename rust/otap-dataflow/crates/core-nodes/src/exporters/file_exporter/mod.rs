...
use otel_arrow_dfe_pdata::{OtapPayloadHelpers, PayloadView};
...
impl FileExporter {
    async fn export_pdata(
        ...
        if let Err(error) =
            view.and_then(|view| encode_payload(view, &mut self.frame, self.config.max_frame_bytes))
        {
...
fn encode_payload(
    ...
    match view {
        ...
        PayloadView::OtapArrowRecords(records) => match records.signal_type() {
            SignalType::Logs => {
